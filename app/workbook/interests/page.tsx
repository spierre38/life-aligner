                                    ...
                                    <div className="flex flex-col sm:flex-row items-center gap-4">
                                        ...
                                        <button
                                            onClick={() => {
                                                ...
                                                const existingList = Array.from(selectedExisting);
                                                const exploringList = Array.from(selectedExploring);
                                                win.document.write(`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <title>My Interests — Tim Collins Framework</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .section-label { font-family: 'Helvetica Neue', Arial, sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; color: #888; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 1px solid #eee; }
    .tags { display: flex; flex-wrap: wrap; gap: 8px; }
    .tag { font-family: 'Helvetica Neue', Arial, sans-serif; font-size: 13px; padding: 6px 14px; border: 1px solid #ddd; border-radius: 100px; color: #333; }
    .tag.exploring { border-color: #bbb; color: #555; font-style: italic; }
...
<body>
  ...
  <div class="section">
    <div class="section-label">Current Interests (${existingList.length})</div>
    <div class="tags">${existingList.map(i => `<span class="tag">${i}</span>`).join('')}</div>
  </div>
  <div class="section">
    <div class="section-label">Interests to Explore (${exploringList.length})</div>
    <div class="tags">${exploringList.map(i => `<span class="tag exploring">${i}</span>`).join('')}</div>
  </div>
  <div class="footer">Generated by timcollinsframework.com &nbsp;&bull;&nbsp; Tim Collins Framework &copy; ${new Date().getFullYear()}</div>
</body>
</html>`);
                                                win.document.close();
                                            }}
