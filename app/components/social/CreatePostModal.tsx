...
import { checkContent, getModerationMessage } from '@/lib/moderation';
...
export default function CreatePostModal({ onClose, onPostCreated }: CreatePostModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const modMsg = getModerationMessage(modResult);
        if (modMsg) {
            setModerationError(modMsg);
            return;
        }
            ...
            <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    ...
                    <div className="mb-6">
                        ...
                        <textarea
                            value={content}
                            onChange={(e) => { setContent(e.target.value); setModerationError(null); }}
                            placeholder="What's on your mind?"
                            rows={6}
                            className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none text-gray-900 placeholder:text-gray-400 ${
                                moderationError ? 'border-red-400 bg-red-50' : 'border-gray-300'
                            }`}
                            required
                        />
                        {moderationError && (
                            <div className="mt-2 flex items-start gap-2 text-sm text-red-600 bg-red-50 border border-red-200 rounded-xl px-3 py-2.5">
