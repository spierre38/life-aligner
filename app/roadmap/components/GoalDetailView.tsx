  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row min-h-screen pt-navbar pb-32">
        ...
        {/* ── Mobile Reflect panel (hidden on lg+) ─────────────────── */}
        <div className="lg:hidden w-full border-t px-4 py-5 space-y-4"
          style={{ borderColor: 'var(--color-border)', background: 'var(--color-surface)' }}
        >
          <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--color-text-dim)' }}>📝 Reflect on this goal</p>
          ...
          <div className="flex gap-2">
            {(['great', 'okay', 'hard'] as const).map(m => (
              <button
                ...
                style={{
                  background: reflectionMood === m
                    ? m === 'great' ? 'rgba(0,200,100,0.18)' : m === 'hard' ? 'rgba(255,80,80,0.18)' : 'var(--color-surface-2)'
                    : 'var(--color-surface-2)',
                  color: reflectionMood === m
                    ? m === 'great' ? '#10b981' : m === 'hard' ? '#f87171' : 'var(--color-text)'
                    : 'var(--color-text-dim)',
                  border: `1px solid ${reflectionMood === m
                    ? m === 'great' ? 'rgba(0,200,100,0.35)' : m === 'hard' ? 'rgba(255,80,80,0.35)' : 'var(--color-border)'
                    : 'var(--color-border)'}`,
                }}
              >
                {m === 'great' ? '✦ Great' : m === 'hard' ? '⊘ Hard' : '~ Okay'}
          ...
          <textarea
            ...
            placeholder="How's this goal going? What's on your mind..."
            rows={4}
            className="w-full rounded-xl p-3 text-sm resize-none focus:outline-none transition-all"
            style={{
              background: 'var(--color-surface-2)',
              border: '1px solid var(--color-border)',
              color: 'var(--color-text)',
          ...
          {goal.reflections && goal.reflections.length > 0 && (
            <div className="space-y-3 pt-2">
              <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--color-text-dim)' }}>Past Entries ({goal.reflections.length})</p>
              {[...goal.reflections].reverse().map(r => (
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {r.mood && (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-medium"
                        style={{
                          background: r.mood === 'great' ? 'rgba(0,200,100,0.15)' : r.mood === 'hard' ? 'rgba(255,80,80,0.15)' : 'var(--color-surface-2)',
                          color: r.mood === 'great' ? '#10b981' : r.mood === 'hard' ? '#f87171' : 'var(--color-text-muted)',
                        }}
                      >
                        {r.mood === 'great' ? '✦ Great' : r.mood === 'hard' ? '⊘ Hard' : '~ Okay'}
                      </span>
